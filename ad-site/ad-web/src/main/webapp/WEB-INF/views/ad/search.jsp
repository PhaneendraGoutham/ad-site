...
<form:form commandName="adSearchDto" action="${searchUrl }" method="GET">
	<article class="center-article-wrapper ui-corner-all">
		...
		<div id="search-params-wrapper" class="clearfix">
			...
			<ul id="filters-menu" class="styled">
				<c:if test="${tagFilter != false }">
					<li id="tag-menu-filter"
						onclick="toggleFilters('tag'); return false;"><spring:message
							code="label.tags"></spring:message></li>
				</c:if>
				<c:if test="${winnerFilter == true }">
					<li id="winner-menu-filter"
						onclick="toggleFilters('winner'); return false;">Zwycięskie</li>
				</c:if>
				<c:if test="${brandFilter != false }">
					<li id="brand-menu-filter"
						onclick="toggleFilters('brand'); return false;"><spring:message
							code="label.brand"></spring:message></li>
				</c:if>
				<c:if test="${placeFilter != false }">
					<li id="place-menu-filter"
						onclick="toggleFilters('place'); return false;"><spring:message
							code="label.place"></spring:message></li>
				</c:if>
				<c:if test="${yearFilter != false }">
					<li id="year-menu-filter"
						onclick="toggleFilters('year'); return false;"><spring:message
							code="label.production.year"></spring:message></li>
				</c:if>
				<li id="rank-vote-menu-filter"
					onclick="toggleFilters('rank-vote'); return false;"><spring:message
						code="label.rank.and.vote.count"></spring:message></li>
			</ul>
			<div id="filters">
				<c:if test="${yearFilter != false }">
				...
				<div id="rank-vote-filters" class="hidden">
					<span class="color-imp"><spring:message code="label.rank"></spring:message>:</span><span
						class="smaller-font"> <spring:message code="label.from"></spring:message>
					</span>
					<form:select id="rank-select-from" path="rankFrom"
						onchange="displayFilter('rank', null, null);" data-target="rank">
					...
					<form:select id="rank-select-to" path="rankTo"
						onchange="displayFilter('rank', null, null);" data-target="rank">
						<form:option value="0">-</form:option>
						<c:forEach var="i" begin="1" end="5">
							<c:choose>
					...
					<span class="color-imp"><spring:message
							code="label.vote.count"></spring:message>:</span> <span
						class="smaller-font"> <spring:message code="label.from"></spring:message>
					</span>
				...
				<c:if test="${placeFilter != false }">
					<div id="place-filters" class="hidden">
						...
						<form:select path="place" id="place-select"
							onchange="displayFilter('place', null, null);"
							data-target="place">
							<form:option value="-1">-</form:option>
							<form:option value="0">
							...
							<form:option value="1">
								<spring:message code="label.waiting"></spring:message>
							</form:option>
						</form:select>
					</div>
				</c:if>
				<c:if test="${winnerFilter == true }">
					<div id="winner-filters" class="hidden">
						<span class="color-imp">Zwycięskie:</span>
						<form:select path="winner" id="winner-select"
							onchange="displayFilter('winner', null, null);"
							data-target="winner">
							<form:option value="">-</form:option>
							<form:option value="0">Nie</form:option>
							<form:option value="1">Tak</form:option>
						</form:select>
					</div>
				</c:if>
				<c:if test="${tagFilter != false }">
								...
								<c:forEach items="${ tags}" var="tag"
									...
									<li id="tag-filter-item-${tag.object.id}"
										...
										<div class="close" data-target="tag"
											data-id="${tag.object.id}">x</div></li>
								</c:forEach>
							</ul>
						</c:forEach>
					</div>
							...
							<ul class="column filters-item-list styled">
								<c:forEach items="${ brands}" var="brand"
									...
									<li id="brand-filter-item-${brand.object.id}"
										...
										data-ui-name="${brand.object.name}"
										data-id="${brand.object.id}">${brand.object.name}</span>
	...
	<article class="center-article-wrapper ui-corner-all">
			...
			<ul id="selected-filters-2" class="styled">
				<c:if test="${placeFilter != false }">
					<li id="place-selected-filters" class="hover-opacity-none"><span
						class="inline-label "><spring:message code="label.place"></spring:message>:</span><span
						class="value"></span>
						<div class="close" data-target="place">x</div></li>
				</c:if>
				<c:if test="${winnerFilter == true }">
					<li id="winner-selected-filters" class="hover-opacity-none"><span
						class="inline-label ">Zwycięskie:</span><span class="value"></span>
						<div class="close" data-target="winner">x</div></li>
				</c:if>
				<c:if test="${yearFilter != false }">
					<li id="year-selected-filters" class="hover-opacity-none"><span
						class="inline-label"><spring:message
								code="label.production.year"></spring:message>:</span><span
						class="value"></span>
						<div class="close" data-target="year">x</div></li>
				</c:if>
				<li id="rank-selected-filters" class="hover-opacity-none"><span
					class="inline-label"><spring:message code="label.rank"></spring:message>:</span><span
					class="value"></span>
		...
		<div id="sort-filters-holder" class="clearfix">
			...
			<ul id="sort-filters" class="styled">
				<li id="rank-sort"
					class="<c:if test="${adSearchDto.orderBy == 'rank'}">on</c:if>"
					onclick="handleSortClick('rank');"><spring:message
						code="label.rank"></spring:message>
					<div
						class="<c:if test="${adSearchDto.order == 'desc'}">arrow-down-white</c:if><c:if test="${adSearchDto.order == 'asc'}">arrow-up-white</c:if>"></div></li>
				<li id="vote-sort"
					class="<c:if test="${adSearchDto.orderBy == 'vote'}">on</c:if>"
					onclick="handleSortClick('vote');"><spring:message
						code="label.vote.count"></spring:message>
					<div
						class="<c:if test="${adSearchDto.order == 'desc'}">arrow-down-white</c:if><c:if test="${adSearchDto.order == 'asc'}">arrow-up-white</c:if>"></div></li>
			</ul>
