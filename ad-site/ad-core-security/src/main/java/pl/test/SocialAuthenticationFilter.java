...
public class SocialAuthenticationFilter extends
	...
	protected Connection<?> addConnection(
			SocialAuthenticationService<?> authService, String userId,
			ConnectionData data) {
		System.out.println("AddConnection2");
		...
		if (connectedUserIds.contains(userId)) {
			// already connected
			return null;
		} else if (!authService.getConnectionCardinality().isMultiUserId()
				&& !connectedUserIds.isEmpty()) {
			return null;
		}
		...
		if (!authService.getConnectionCardinality().isMultiProviderUserId()) {
			List<Connection<?>> connections = repo.findConnections(data
					.getProviderId());
	...
	private void addConnection(
		...
		System.out.println("AddConnection1");
	...
	private void updateConnections(SocialAuthenticationService<?> authService,
		...
		if (updateConnections) {
			System.out.println("UpdateConnections");
			String userId = ((SocialUserDetails) success.getPrincipal())
					.getUserId();
